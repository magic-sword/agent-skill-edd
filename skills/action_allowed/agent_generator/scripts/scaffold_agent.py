...
"""
    ...
    template_path = os.path.join(assets_dir, "assets", "agent_template.py")
    
    if not os.path.exists(template_path):
        print(f"Error: Template agent_template.py not found at {template_path}")
        sys.exit(1)
...
{edges_code}
    ...
    async def run_internal_workflow(input_str: str) -> str:
        \"\"\"エージェントに定義された固定ステップのワークフローを実行します。\"\"\"
        sub_app = App(
            name="{dir_name}_sub_workflow",
            root_agent=workflow
        )
    ...
    return Agent(
        ...
        instruction="あなたは {name} エージェントです。タスクを遂行するために、必要に応じて run_internal_workflow ツールを呼び出して実行してください。",
...
"""
    ...
    run_agent_path = os.path.join(workflow_dir, "agent.py")
    with open(run_agent_path, "w", encoding="utf-8") as f:
        f.write(run_agent_content)
        
    # 5. Write [workflow_name].test.json
    test_template_path = os.path.join(assets_dir, "assets", "agent_test_set_template.json")
    ...
    replaced_test = (test_template
                     ...
                     .replace("{{arg_value_1}}", test_cases.get("arg_value_1", f"python agents/run.py --agent-script {rel_output_path}/agent.py --input test"))
                     .replace("{{negative_test_input_1}}", test_cases.get("negative_test_input_1", "How to cook ramen?"))
                     .replace("{{negative_expected_output_1}}", test_cases.get("negative_expected_output_1", "This agent does not support general questions.")))
                     
    test_json_path = os.path.join(workflow_dir, f"{dir_name}.test.json")
    with open(test_json_path, "w", encoding="utf-8") as f:
        f.write(replaced_test)
        
    print(f"Successfully scaffolded agent '{name}' in: {workflow_dir}")
