...
class E2eTrajectoryPattern(BaseEvaluationPattern):
    ...
    Performs AST analysis on run_agent.py and validates E2E outputs and tool trajectories.
    """
    ...
    def evaluate(self, context: Any) -> Tuple[bool, List[str]]:
        ...
        print(f"  - Actual parsed trajectory (from run_agent.py): {actual_trajectory}")
        ...
        for case in context.eval_cases:
            ...
            if not actual_trajectory:
                issues.append("Trajectory validation failed: Actual node execution trajectory is empty in run_agent.py.")
                trajectory_pass = False
                break
            ...
            if not internal_ok:
                issues.append("Tool Trajectory validation failed: Node output tool 'write_node_output' is never called in run_agent.py.")
                trajectory_pass = False
                break
