...
description: |
  エージェントライブラリ内のエージェントの品質および挙動の評価を実行します。
  ...
  エージェントでは ANY_ORDER による曖昧な評価を禁止する整合性チェックを含みます。
version: 1.0.0
license: MIT
...
エージェントライブラリの各エージェントが、指定されたTierの基準および ADK v2.3.0 の E2E・Tool Trajectory 要件を満たしているかを検証します。

## When to use
- エージェントの品質チェックを行いたいとき。
- エージェントを上のTierに昇格させる前段階で、事前評価を実行したいとき。

## When NOT to use
- エージェントの新規生成（Scaffold）を行うとき（`agent-generator` を使用）。
- エージェントの物理的なTier移動や整理を行うとき（`agent-manager` を使用）。

## Workflow

### 1. 評価の実行
評価対象のエージェント名と、目指すターゲットTier（デフォルトは `draft_only`）を指定して評価スクリプトを実行します。

* **実行コマンド**:
  ```powershell
  python skills/action_allowed/agent_evaluator/scripts/evaluate_agent.py --workflow <agent-name> --target-tier <target-tier>
  ```
  ※ `target-tier` には `draft_only`, `read_only`, `action_allowed` のいずれかを指定します。
  ※ 引数 `--workflow` は `--agent` でも呼び出し可能です。
